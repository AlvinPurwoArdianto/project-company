...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold">Tambah Testimoni</h4>
            <p class="text-muted mb-0">Masukkan testimoni dan rating pengguna</p>
        </div>
            ...
            <form action="{{ route('testimoni.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="testimoni" id="testimoni" rows="3"
                        ...
                        placeholder="Tulis testimoni Anda">{{ old('testimoni') }}</textarea>
                    @error('testimoni')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <div class="star-rating d-flex align-items-center gap-1" style="font-size: 1.8rem;">
                        @for ($i = 1; $i <= 5; $i++)
                            ...
                            <label for="rating{{ $i }}" style="cursor:pointer; color:{{ old('rating', 0) >= $i ? '#ffc107' : '#e4e5e9' }}; margin-bottom:0;">
                                &#9733;
                            </label>
                        @endfor
                    </div>
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary">
                        <i class="bx bx-save me-1"></i> Simpan Testimoni
                    </button>
                </div>
            </form>
        </div>
